 ...
 */
void PQC_HQC192_PORTABLE_code_encode(uint8_t *em, const uint8_t *m) {
    uint8_t tmp[VEC_N1_SIZE_BYTES] = {0};

    PQC_HQC192_PORTABLE_reed_solomon_encode(tmp, m);
    PQC_HQC192_PORTABLE_reed_muller_encode(em, tmp);
 ...
 */
void PQC_HQC192_PORTABLE_code_decode(uint8_t *m, const uint8_t *em) {
    uint8_t tmp[VEC_N1_SIZE_BYTES] = {0};

    PQC_HQC192_PORTABLE_reed_muller_decode(tmp, em);
    PQC_HQC192_PORTABLE_reed_solomon_decode(m, tmp);
