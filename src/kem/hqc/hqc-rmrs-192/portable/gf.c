 ...
 */
uint16_t PQC_HQC192_PORTABLE_gf_mul(uint16_t a, uint16_t b) {
    uint16_t mask;
    mask = (uint16_t) (-((int32_t) a) >> 31); // a != 0
    mask &= (uint16_t) (-((int32_t) b) >> 31); // b != 0
    return mask & gf_exp[PQC_HQC192_PORTABLE_gf_mod(gf_log[a] + gf_log[b])];
}
 ...
 */
uint16_t PQC_HQC192_PORTABLE_gf_square(uint16_t a) {
    int16_t mask = (uint16_t) (-((int32_t) a) >> 31); // a != 0
    return mask & gf_exp[PQC_HQC192_PORTABLE_gf_mod(2 * gf_log[a])];
}
 ...
 * @param[in] a Element of GF(2^PARAM_M)
 */
uint16_t PQC_HQC192_PORTABLE_gf_inverse(uint16_t a) {
    int16_t mask = (uint16_t) (-((int32_t) a) >> 31); // a != 0
 ...
 */
uint16_t PQC_HQC192_PORTABLE_gf_mod(uint16_t i) {
