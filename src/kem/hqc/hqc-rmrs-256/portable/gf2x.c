...
//static uint32_t fast_convolution_mult(const uint64_t *A, const uint32_t *vB, uint64_t *C, const uint16_t w, AES_XOF_struct *ctx)
    ...
    for (i = 0; i < weight; i++) {
        ...
        for (j = 0; j < VEC_N_SIZE_64 + 1; j++) {
            tmp = PQC_HQC256_PORTABLE_load8(res);
            PQC_HQC256_PORTABLE_store8(res, tmp ^ pt[j]);
            res += 8;
        }
    }
}
 ...
 */
void PQC_HQC256_PORTABLE_vect_mul(uint64_t *o, const uint32_t *a1, const uint64_t *a2, uint16_t weight, AES_XOF_struct *ctx) {
    uint64_t tmp[2 * VEC_N_SIZE_64 + 1] = {0};

    fast_convolution_mult((uint8_t *) tmp, a1, a2, weight, ctx);
    PQC_HQC256_PORTABLE_load8_arr(tmp, 2 * VEC_N_SIZE_64 + 1, (uint8_t *) tmp, sizeof(tmp));
    reduce(o, tmp);
}
