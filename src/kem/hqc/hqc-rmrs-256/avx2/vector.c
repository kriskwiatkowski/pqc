 ...
 */
void PQC_HQC256_vect_set_random_fixed_weight(AES_XOF_struct *ctx, uint64_t *v, uint16_t weight) {
 ...
 * @param[in] ctx Pointer to the context of the seed expander
 */
void PQC_HQC256_vect_set_random(AES_XOF_struct *ctx, uint64_t *v) {
    uint8_t rand_bytes[VEC_N_SIZE_BYTES] = {0};

    seedexpander(ctx, rand_bytes, VEC_N_SIZE_BYTES);

    PQC_HQC256_load8_arr(v, VEC_N_SIZE_64, rand_bytes, VEC_N_SIZE_BYTES);
 ...
 */
void PQC_HQC256_vect_add(uint64_t *o, const uint64_t *v1, const uint64_t *v2, uint32_t size) {
 ...
 */
uint8_t PQC_HQC256_vect_compare(const uint8_t *v1, const uint8_t *v2, uint32_t size) {
    uint64_t r = 0;
 ...
 */
void PQC_HQC256_vect_resize(uint64_t *o, uint32_t size_o, const uint64_t *v, uint32_t size_v) {
