...
int PQC_HQC256_crypto_kem_keypair(unsigned char *pk, unsigned char *sk) {
    ...
    PQC_HQC256_hqc_pke_keygen(pk, sk);
    return 0;
}
...
int PQC_HQC256_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk) {
    ...
    // Encrypting m
    PQC_HQC256_hqc_pke_encrypt(u, v, m, theta, pk);
    ...
    PQC_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES, VEC_N_SIZE_BYTES, u, VEC_N_SIZE_64);
    PQC_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES, VEC_N1N2_SIZE_BYTES, v, VEC_N1N2_SIZE_64);
    sha512(ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES);

    // Computing ciphertext
    PQC_HQC256_hqc_ciphertext_to_string(ct, u, v, d);
...
int PQC_HQC256_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    ...
    PQC_HQC256_hqc_ciphertext_from_string(u, v, d, ct);

    // Retrieving pk from sk
    memcpy(pk, sk + SEED_BYTES, PUBLIC_KEY_BYTES);

    // Decryting
    PQC_HQC256_hqc_pke_decrypt(m, u, v, sk);

    // Computing theta
    sha3_512(theta, m, VEC_K_SIZE_BYTES);

    // Encrypting m'
    PQC_HQC256_hqc_pke_encrypt(u2, v2, m, theta, pk);
    ...
    // Computing shared secret
    memcpy(mc, m, VEC_K_SIZE_BYTES);
    PQC_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES, VEC_N_SIZE_BYTES, u, VEC_N_256_SIZE_64);
    PQC_HQC256_store8_arr(mc + VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES, VEC_N1N2_SIZE_BYTES, v, VEC_N1N2_SIZE_64);
    sha512(ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES);

    // Abort if c != c' or d != d'
    result = PQC_HQC256_vect_compare((uint8_t *)u, (uint8_t *)u2, VEC_N_SIZE_BYTES);
    result |= PQC_HQC256_vect_compare((uint8_t *)v, (uint8_t *)v2, VEC_N1N2_SIZE_BYTES);
    result |= PQC_HQC256_vect_compare(d, d2, SHA512_BYTES);
