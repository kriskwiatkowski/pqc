...
**************************************************/
void PQCLEAN_KYBER512_AVX2_polyvec_compress(uint8_t r[KYBER_POLYVECCOMPRESSEDBYTES + 2], polyvec *a) {
    size_t i;
...
**************************************************/
void PQCLEAN_KYBER512_AVX2_polyvec_decompress(polyvec *r, const uint8_t a[KYBER_POLYVECCOMPRESSEDBYTES + 12]) {
    size_t i;
...
/*************************************************
* Name:        PQCLEAN_KYBER512_AVX2_polyvec_basemul_acc_montgomery
*
...
void PQCLEAN_KYBER512_AVX2_polyvec_basemul_acc_montgomery(poly *r, const polyvec *a, const polyvec *b) {
    ...
    for (i = 1; i < KYBER_K; i++) {
        PQCLEAN_KYBER512_AVX2_poly_basemul_montgomery(&tmp, &a->vec[i], &b->vec[i]);
        PQCLEAN_KYBER512_AVX2_poly_add(r, r, &tmp);
    }
}
